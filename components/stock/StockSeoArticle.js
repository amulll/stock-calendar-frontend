...
export default function StockSeoArticle({ info, latestDividend, metrics, isUpcomingEvent }) {
  const { stock_name, stock_code, daily_price } = info;
  const { cash_dividend, stock_dividend, pay_date, ex_date } = latestDividend || {};
  const calculatorCashDividend = latestDividend?.adjusted_cash_dividend ?? cash_dividend;
    ...
    <article className="prose prose-slate max-w-none">
      ...
      <p className="text-slate-600 leading-relaxed mb-4">
        {isUpcomingEvent ? (
          ...
          <>
            最近一期事件的<strong>{exLabel}為 {ex_date || "尚未公告"}</strong>，
            已是完成的歷史事件；{isStockOnly ? "配發日" : "配息日"}為{" "}
            <strong>{pay_date || "尚未公告"}</strong>。
          </>
        )}
        {avgFillDays && (
          <span>
            回顧本站已完成評估的紀錄，{stock_name} 成功填息事件的平均天數約為{" "}
      ...
      {isUpcomingEvent && (
        <>
          ...
          <p className="text-slate-600 leading-relaxed mb-4">
            ...
            <strong>{Number(isStockOnly ? stock_dividend : calculatorCashDividend).toFixed(3)} 元</strong>，自動計算出您可領取的總股利金額。
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
        <p>
          ...
          {isUpcomingEvent ? (
            ...
            <>
              這是已完成的歷史事件。是否實際領到股利取決於當時是否符合持有條件，
              不能直接用目前持股數推算。
            </>
          )}
        </p>
