...
export function renderShareCard(canvas, data) {
  ...
  cursorY += 24;
  drawMonthlyChart(ctx, data.monthly, left, cursorY, right - left, 400);

  // 頁尾
  const footY = cardY + cardH - pad + 6;
  ctx.fillStyle = COLORS.ink;
  ctx.font = font(700, 32);
  ctx.textAlign = "left";
  ctx.fillText("ugoodly.com", left, footY);
  ctx.fillStyle = COLORS.inkMuted;
  ctx.font = font(400, 26);
  ctx.textAlign = "right";
  ctx.fillText("台股股利日曆 · 試算僅供參考", right, footY);
}
